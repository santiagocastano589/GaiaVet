...
export const Header = ({ title, classN }) => {
  ...
  return (
    <header className={`ola w-full sm:w-[40rem] h-28 bg-cover flex justify-center z-40 fixed transition-transform duration-300 ${showHeader ? 'transform translate-y-0' : 'transform -translate-y-full'}`}>
      <div className='w-4/5 flex items-center justify-between sm:w-[90%]'>
        <Link to={"/"} className='w-16 flex items-center sm:w-12'>
          <img className='bg-white p-1 rounded-full' src={logo} alt="Logo" />
          <h2 className='text-3xl font-gorditas p-3 sm:text-2xl sm:p-2'>GaiaVet</h2>
        </Link>
        <h2 className={`font-bold text-stroke-2-white font-gorditas mt-16 ${classN} sm:text-6xl`}>
          {title}
        </h2>        
        <div className='flex'>
          <img className='cursor-pointer w-10 sm:w-8' onClick={controlOpenMenu} src={menu} alt="" />
