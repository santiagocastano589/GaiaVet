...
export const Citas2 = () => {
  ...
  const servicesList = [
    {
      name: 'Baño',
      image: 'https://res.cloudinary.com/dxg8bqs9x/image/upload/v1726602549/aseo-de-mascotas_i24la6.png',
    },
    {
      name: 'Consulta',
      image: 'https://res.cloudinary.com/dxg8bqs9x/image/upload/v1726602566/consulta_awptvb.png',
    },
    {
      name: 'Peluqueria',
      image: 'https://res.cloudinary.com/dxg8bqs9x/image/upload/v1726602569/peluqueria_xdftj2.png',
    },
    {
      name: 'Guarderia',
      image: 'https://res.cloudinary.com/dxg8bqs9x/image/upload/v1726602555/calendario_g7qrhl.png',
    },
  ];
  ...
  const filterAvailableTimes = (occupiedTimes) => {
    ...
    let filteredTimes = initialAvailableTimes.filter(time => !occupiedTimes.includes(time));
  
    // Si la fecha seleccionada es hoy, filtrar las horas
    if (selectedDateObj.toDateString() === today.toDateString()) {
      const currentTime = format(today, 'HH:mm:ss');
      
      filteredTimes = filteredTimes.filter(time => time > currentTime);
    }
  
    if (filteredTimes.length === 0) {
      Swal.fire('No hay horas disponibles', 'No hay horas disponibles para la fecha seleccionada.', 'info');
    }
    ...
    const fetchEmployees = async () => {
      if (selectedService && selectedPet) {
        try {
          ...
          const response = await fetch(`https://gaiavet-back.onrender.com/employeServices/${(selectedService.name)}`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${authToken}`,
  ...
  const handleServiceSelect = async (serviceName) => {
    const selected = servicesList.find((service) => service.name === serviceName);
    setSelectedService(selected);

    // Restablece la selección de mascota y empleado
    setSelectedPet(null);
                  ...
                  <div className='w-[90%] flex justify-evenly flex-wrap'>
                    {selectedService ? (
                      <div className='w-1/5 mx-2 flex flex-col items-center rounded-full p-5 justify-center'>
                        <img className='rounded-full object-cover w-[10vw] h-[10vw] bg-white' src={selectedService.image} alt={selectedService.name} />
                        <p className='text-3xl'>{selectedService.name}</p>
                      ...
                      servicesList.map((service) => (
                        <div key={service.name} className='w-1/5 mx-2 flex flex-col items-center rounded-full p-5 justify-center hover:-translate-y-4 hover:bg-emerald-950 duration-500' onClick={() => handleServiceSelect(service.name)}>
                          <img className='rounded-full object-cover w-[10vw] h-[10vw] bg-white' src={service.image} alt={service.name} />
